...
export function renderDeadline(p){
  ...
  if(p.dateStart && p.dateEnd){
    const cal = buildDeadlineCal(p);
    if(cal) box.appendChild(cal);
  }
}

const _CAL_MONTHS=['Gennaio','Febbraio','Marzo','Aprile','Maggio','Giugno','Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre'];
const _CAL_DAYS=['L','M','M','G','V','S','D'];

function buildDeadlineCal(p){
  const start=new Date(p.dateStart); const end=new Date(p.dateEnd);
  ...
  while(cur.getFullYear()*12+cur.getMonth() <= end.getFullYear()*12+end.getMonth() && months.length<6){
    months.push(new Date(cur));
    cur.setMonth(cur.getMonth()+1);
  }

  const wrap=document.createElement('div');
  wrap.className='deadline-cal';
  ...
  months.forEach(month=>{
    ...
    block.className='deadline-cal-month';
    const hdr=document.createElement('div');
    hdr.className='deadline-cal-hdr';
    hdr.textContent=`${_CAL_MONTHS[m]} ${y}`;
    block.appendChild(hdr);

    const grid=document.createElement('div');
    grid.className='deadline-cal-grid';
    _CAL_DAYS.forEach(d=>{
      const cell=document.createElement('div');
      cell.className='deadline-cal-day-name';
      cell.textContent=d;
    ...
    for(let d=1;d<=daysInMonth;d++){
      ...
      const cell=document.createElement('div');
      cell.className='deadline-cal-day';
      cell.textContent=d;
      if(t===start.getTime()) cell.classList.add('is-start');
      else if(t===end.getTime()) cell.classList.add('is-end');
      else if(t===today.getTime()) cell.classList.add('is-today');
      else if(date>=start&&date<=end&&date<today) cell.classList.add('past');
      else if(date>=start&&date<=end) cell.classList.add('in-range');
      grid.appendChild(cell);
    }
  ...
  leg.className='deadline-cal-legend';
  leg.innerHTML=`
    <span class="deadline-cal-leg"><span class="deadline-cal-dot" style="background:var(--moss)"></span>Inizio</span>
    <span class="deadline-cal-leg"><span class="deadline-cal-dot" style="background:var(--sky)"></span>Oggi</span>
    <span class="deadline-cal-leg"><span class="deadline-cal-dot" style="background:var(--coral)"></span>Deadline</span>
    <span class="deadline-cal-leg"><span class="deadline-cal-dot" style="background:var(--sand2)"></span>Trascorsi</span>`;
