...
// ── SABBIA DISATTIVATA ──
// Animazione decorativa sul canvas #home-sand (mai richiesta dall'utente per
// nessuna funzione: puro effetto visivo). Misurata a ~559 ms di CPU al
// secondo sulla sola Home con CPU rallentata 4× (telefono di fascia media) —
// più di mezzo core, in continuazione, per un effetto decorativo. Anche dopo
// un'ottimizzazione (30fps, densità 1, fillRect raggruppati) restava a
// ~192 ms/s, quindi è stata spenta del tutto.
//
// Il codice completo (sia la versione originale a 60fps sia quella
// ottimizzata) resta in js/home-sandstorm-60fps.archive.js: se in futuro la
// si rivuole, da lì si recupera pari pari.
//
// window._resumeSand resta undefined di proposito: main.js ed evening.js la
// richiamano già in forma protetta (`if(window._resumeSand) ...`), quindi
// disattivarla qui non richiede toccare nessun altro file.
let _sandStarted = false;
export function startSandstorm(){
  _sandStarted = true; // no-op: vedi commento sopra
}
